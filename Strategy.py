...
Problem:
- We are building a complex system which have has to deal with a dynamic problem  that needs more that one algorithm to be implemented
- the choice of the algorithm to use depends heavily on the client request -> Different algorithms will be appropriate at different times
- We need to hard code all the needed algorithms in the part of code used to handle the client's request
- Everytime we need add a new algorithm or maintain some of them, we need to break the client code and modify it
solution:
- encapsulate every algorithm in a separate class, the encapsulated algorithm is then called a concretestrategy
- make a common interface (we call strategy) to all (implemented by) classes that encapsulate the desired algorithms
- The client ( we call it the context ) can call the algorithms dynamically ( when it needs to) via the interface
- if the client (context) needs its data to be accessed then it can provide an interface to the Strategy
Consequences:
- the algorithms can be easily reused
- The algorithms can be easily upgraded and enriched using inheritence
...
- the unneeded amount of communication between the strategy interface and the context: sometimes the client use initiate the
  ...
  to, avoid this, we need to increase coupling between the client and strategy which is a bad practise
"""
"""implementation 
...
- Now are going to implement a common interface of the concrete strategies
"""
